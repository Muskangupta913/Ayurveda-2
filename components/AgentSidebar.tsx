...
interface NavItem {
  ...
  description?: string;
  children?: NavItem[];
  permissionKey?: string; // optional permission for each menu
}
...
const navItems: NavItem[] = [
  ...
  {
  label: "assigned lead",
  path: "/agent/assigned-leads", // <-- changed
  icon: "📋",
  description: "Overview & metrics",
...
const AgentSidebar: FC<AgentSidebarProps> = ({ className }) => {
  ...
  useEffect(() => {
    const fetchPermissions = async () => {
      try {
        const token = localStorage.getItem("agentToken"); // adjust if cookie
        if (!token) return;
...
const filterByPermissions = (items: NavItem[]): NavItem[] => {
  return items
    .filter((item) => {
      // Force assigned-leads to always show
  ...
  const filteredNavItems = filterByPermissions(navItems);
          ...
          <nav className="flex-1 overflow-y-auto custom-scrollbar px-4 py-6 min-h-0">
            ...
            <div className="space-y-1">
              {(filteredNavItems || navItems).map((item) => {
                ...
                // Regular (non-dropdown) item with path
                if (item.path) {
                  return (
                ...
                // Parent item without path (like "Lead" from second document)
                return (
                  <div key={item.label}>
                    <div className="px-3 py-2 font-semibold text-gray-700 flex items-center space-x-3">
                      <div className="text-lg p-2 rounded-lg bg-gray-100 text-gray-600">
                        {item.icon}
                      </div>
                    ...
                    {item.children && (
                      <div className="ml-6 space-y-1 mt-2">
                        {item.children.map((child) => {
                          const childActive = child.path && router.pathname === child.path;
                          return (
                            <Link key={child.label} href={child.path!}>
                              <div
                                className={clsx(
                                  "block px-3 py-2 text-sm rounded-md flex items-center space-x-2",
                                  {
                                    "bg-[#2D9AA5] text-white": childActive,
                                    "text-gray-700 hover:bg-gray-50": !childActive,
                                  }
                                )}
                              >
            ...
            <nav className="flex-1 overflow-y-auto px-4 py-4 min-h-0">
              ...
              <div className="space-y-1">
                {(filteredNavItems || navItems).map((item) => {
                  ...
                  return (
                    <div key={item.label}>
                      <div className="px-3 py-2 font-semibold text-gray-700 flex items-center space-x-3 touch-manipulation">
                        <div className="text-lg p-2 rounded-lg bg-gray-100 text-gray-600">
                          {item.icon}
                        </div>
                        <span>{item.label}</span>
                          ...
                          {item.children.map((child) => {
                            ...
                            return (
                              <Link key={child.label} href={child.path!}>
                                <div
                                  className={clsx(
                                    "block px-3 py-2 text-sm rounded-md flex items-center space-x-2 touch-manipulation active:scale-98",
                                    {
                                      "bg-[#2D9AA5] text-white": childActive,
                                      "text-gray-700 hover:bg-gray-50 active:bg-gray-100": !childActive,
                                    }
                                  )}
                                  onClick={handleItemClick}
                                >

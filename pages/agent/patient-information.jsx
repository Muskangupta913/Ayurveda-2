...
function PatientFilterUI() {
  ...
  const fetchPatients = async () => {
    ...
    setLoading(true);
    try {
      const { data } = await axios.get("/api/agent/get-patient-registrations", { params: filters, headers: { Authorization: `Bearer ${token}` } });
  ...
  const handleUpdate = (id) => router.push(`/agent/update-patient-info/${id}`);
...
PatientFilterUI.getLayout = function PageLayout(page) { return <AgentLayout>{page}</AgentLayout>; };
const ProtectedDashboard = withAgentAuth(PatientFilterUI);

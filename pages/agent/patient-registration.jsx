...
const InvoiceManagementSystem = () => {
  ...
  useEffect(() => {
    ...
    if (!token) return;
    fetch("/api/agent/patient-registration", {
  ...
  const fetchEMRData = useCallback(async () => {
    ...
    try {
      setFetching(true);
      const token = typeof window !== "undefined" ? localStorage.getItem("agentToken") : null;
      const res = await fetch(`/api/agent/patient-registration/${formData.emrNumber}`, {
        headers: { Authorization: `Bearer ${token}` }
      });
...
const handleSubmit = useCallback(async () => {
  ...
  setConfirmModal({
    ...
    action: async () => {
      try {
        const token = typeof window !== "undefined" ? localStorage.getItem("agentToken") : null;
        const res = await fetch("/api/agent/patient-registration", {
          method: "POST",
          headers: { 
        ...
        if (res.ok && data.success) {
          ...
          router.push("/agent/patient-information"); 
        } else {
...
const ProtectedDashboard = withAgentAuth(InvoiceManagementSystem);

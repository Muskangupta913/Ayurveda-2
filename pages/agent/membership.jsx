...
const MembershipModal = ({ isOpen, onClose }) => {
  ...
  const fetchByEmr = useCallback(async (overrideEmr) => {
    ...
    try {
      setFetching(true);
      const res = await fetch(`/api/agent/patient-registration/${emrToFetch}`, {
  ...
  useEffect(() => {
    ...
    handle = setTimeout(async () => {
      try {
        const res = await fetch(`/api/agent/emr-search?q=${encodeURIComponent(q)}`, {
          headers: { Authorization: `Bearer ${token}` }
        });
  ...
  const saveMembership = async () => {
    ...
    // Check permission - this is handled by the parent component's permissions state
    // The button is already hidden if canCreate is false
    try {
...
function MembershipPage() {
  ...
  const fetchPermissions = async () => {
    ...
    console.log("Fetching permissions with token:", token.substring(0, 20) + "...");
    ...
    try {
      ...
      if (!res.ok) {
        ...
        if (res.status === 401) {
          ...
          if (freshToken && freshToken !== token) {
            ...
            // Retry with fresh token if different
            return;
          }
        }
      ...
      if (data.success && data.data) {
        ...
        // Find staff_management module (Membership is a submodule under it, like Add Vendor)
        ...
        if (modulePermission) {
          ...
          // Check for "Membership" submodule (like VendorForm checks for "Add Vendor")
          const membershipSubModule = modulePermission.subModules?.find(
            (sm) => sm.name === "Membership" || sm.name?.toLowerCase() === "membership"
          );
          
          console.log("Membership SubModule found:", membershipSubModule);
          
          // If submodule exists, use its actions, otherwise use module-level actions
          const effectiveActions = membershipSubModule?.actions || actions;
          
          console.log("Effective Actions (Membership):", effectiveActions);
          
          // If actions.all is true, grant all permissions
          // Otherwise, check individual action flags (exact same logic as create-offer)
          ...
          console.log("Membership Final Permissions:", {
            canCreate: hasAll || effectiveActions.create === true,
            canUpdate: hasAll || effectiveActions.update === true,
            canDelete: hasAll || effectiveActions.delete === true,
            canRead: hasAll || effectiveActions.read === true,
            hasSubModule: !!membershipSubModule,
          });
        } else {
        ...
        console.log("Membership API failed or no data:", data);
        setPermissions({
          canCreate: false,
          canUpdate: false,
          canDelete: false,
      ...
      console.error("Error fetching membership permissions:", err);
  ...
  useEffect(() => {
    // Get fresh token on mount and when component updates
    const token = getToken();
    if (token) {
      fetchPermissions();
    } else {
  ...
  useEffect(() => {
    // Fetch memberships after permissions are loaded
    if (permissionsLoaded) {
      fetchMemberships();
    }
  }, [permissionsLoaded, permissions.canRead, fetchMemberships]);
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-gray-50 to-gray-100 p-4 sm:p-6 flex items-center justify-center">
        <div className="text-center bg-white p-8 rounded-lg shadow-md max-w-md">
          ...
          <p className="text-gray-600">You don't have permission to view memberships.</p>
        </div>
      </div>
    );
  }
                    ...
                    {permissions.canDelete && (
                      <button 
                        onClick={async () => {
                          ...
                          try {
                            const token = getToken();
                            const res = await fetch(`/api/agent/members?membershipId=${item._id}`, {
                              method: 'DELETE',
                              headers: { Authorization: `Bearer ${token}` }
                            });
                            const json = await res.json();
                            if (res.ok && json.success) {
                              ...
                              alert(json.message || 'Failed to delete membership');
                            }
                          } catch (e) {
                            alert('Error deleting membership');
                          }
                        }}
                        className="flex-1 px-3 py-2 bg-red-50 text-red-700 rounded-lg font-medium text-sm hover:bg-red-100 transition-colors"
                      >
                        Delete
                      </button>
                    )}
...
const Protected = withAgentAuth(MembershipPage);

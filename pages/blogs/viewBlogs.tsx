 ...
 return (
    <div className="min-h-screen bg-gradient-to-br from-white via-slate-50 to-slate-100">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
        {/* Main Heading */}
        <h1 className="text-4xl md:text-5xl font-extrabold text-center mb-4 bg-gradient-to-r from-slate-800 via-teal-600 to-slate-700 bg-clip-text text-transparent tracking-tight">
        ...
        <div className="bg-white/90 backdrop-blur-xl border border-teal-100 rounded-2xl p-8 shadow-xl mb-8">
          <div className="relative max-w-2xl mx-auto mb-6">
            ...
            <input
              ...
              className="w-full pl-14 pr-14 py-4 bg-white/95 border-2 border-teal-200 rounded-2xl text-gray-700 font-medium placeholder-gray-400 focus:border-teal-500 focus:ring-4 focus:ring-teal-100 focus:outline-none transition-all duration-300 shadow-sm"
            />
            {searchTerm && (
              <button
          ...
          <div className="flex justify-center gap-3">
            <button
              ...
              className={`flex items-center gap-2 px-6 py-3 rounded-full font-semibold text-sm transition-all duration-300 ${sortBy === "latest"
            ...
            <button
              ...
              className={`flex items-center gap-2 px-6 py-3 rounded-full font-semibold text-sm transition-all duration-300 ${sortBy === "popular"
                ? 'bg-teal-600 text-white shadow-lg border-2 border-teal-600'
                : 'bg-white/80 text-gray-600 border-2 border-teal-200 hover:bg-teal-50 hover:text-teal-700 hover:border-teal-300'
                }`}
            >
        ...
        <div className="text-center mb-8">
          {searchTerm && (
            <div className="inline-flex items-center gap-2 bg-white/90 border border-teal-200 text-teal-800 px-6 py-3 rounded-full font-semibold text-sm backdrop-blur-md shadow-lg">
              <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        {!blogs.length ? (
          <div className="text-center py-20">
            <div className="w-12 h-12 border-4 border-teal-200 border-l-teal-600 rounded-full animate-spin mx-auto mb-6"></div>
            <p className="text-gray-600 text-lg font-medium">Loading articles...</p>
          </div>
        ) : filteredBlogs.length === 0 ? (
          <div className="text-center py-20 bg-white/60 rounded-2xl mx-auto max-w-2xl">
            <div className="text-6xl mb-6">📝</div>
            <h3 className="text-2xl font-bold text-gray-800 mb-4">No articles found</h3>
            <p className="text-gray-600 mb-6 text-lg">We couldn't find any articles matching your search criteria.</p>
            <button
              onClick={handleClearSearch}
              className="bg-gradient-to-r from-teal-600 to-teal-700 text-white px-8 py-3 rounded-full font-semibold hover:from-teal-700 hover:to-teal-800 transition-all duration-200 transform hover:scale-105"
            >
              View All Articles
            </button>
          </div>
            ...
            <div className="space-y-8">
              {currentBlogs.map((blog, index) => {
                ...
                return (
                  <article
                    key={blog._id}
                    className="bg-white border border-teal-100 rounded-2xl overflow-hidden transition-all duration-500 hover:shadow-2xl hover:-translate-y-2 hover:border-teal-200 shadow-lg animate-fade-in-up"
                    style={{ animationDelay: `${index * 0.1}s` }}
                  >
                    <div className="flex flex-col lg:flex-row gap-8 items-start">
                      {/* Image Section - Always show either real image or placeholder */}
                      <div className="flex-shrink-0 p-6 pb-0 lg:pb-6">
                        {blogImage ? (
                          // Show actual uploaded image
                          <img
                            src={blogImage}
                            alt={blog.title}
                            className="w-full lg:w-80 h-48 lg:h-52 object-contain rounded-2xl shadow-lg transition-transform duration-300 hover:scale-105 bg-gray-50"
                          />
                        ) : (
                          // Show placeholder with "ZEVA Blogs" text
                          <div className="w-full lg:w-80 h-48 lg:h-52 rounded-2xl shadow-lg transition-transform duration-300 hover:scale-105 bg-gradient-to-br from-teal-400 via-teal-500 to-teal-600 flex items-center justify-center relative overflow-hidden">
                            {/* Background Pattern */}
                      ...
                      <div className="flex-1 p-6 lg:py-8">
                        <h2 className="text-2xl lg:text-3xl font-bold leading-tight text-gray-900 mb-4 bg-gradient-to-r from-gray-900 to-gray-700 bg-clip-text text-transparent">
                          {blog.title}
                        </h2>

                        <div className="flex items-center gap-3 mb-5 text-sm">
                          <div className="inline-flex items-center gap-2 bg-gradient-to-r from-teal-50 to-teal-100 text-teal-800 px-3 py-2 rounded-full font-semibold text-xs">
                        ...
                        <div className="text-gray-600 leading-relaxed text-base mb-6">
                          {parse(paragraphs)}
                        </div>

                        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 pt-5 border-t border-teal-50">
                          <div className="flex items-center gap-4">
                            {/* Simple Thumb Button with Popup */}
                            <div className="relative">
                              <button
                                ...
                                className={`group flex items-center gap-2 px-4 py-2 rounded-full font-semibold text-sm transition-all duration-200 ${blog.liked
                              ...
                              >
                                {/* Heart / Like Icon */}
                                <svg
                                  className={`w-5 h-5 transition-all duration-200 hover:scale-105 ${blog.liked ? "text-red-500" : "text-gray-400 hover:text-red-400"
                                    }`}
         ...
         0 0 0 0-7.78z"
                            ...
                            <button
                              ...
                              className="group flex items-center gap-2 px-4 py-2 rounded-full font-semibold text-sm bg-gray-50 text-gray-600 hover:bg-green-50 hover:text-green-600 border border-gray-200 hover:border-green-200 transition-all duration-200"
                            >
                              <svg viewBox="0 0 24 24" className="w-5 h-5 text-gray-500 hover:text-blue-500 transition-colors duration-200">
                              ...
                              <span className="hidden sm:inline text-xs font-medium group-hover:text-green-600"></span>
                            </button>
                          </div>

                          <Link href={`/blogs/${blog._id}`}>
                            <button className="inline-flex items-center gap-2 px-6 py-2.5 bg-teal-600 text-white font-semibold rounded-full shadow-md hover:bg-teal-700 hover:shadow-lg transform hover:scale-105 transition-all duration-200">
                              Read More
                              <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  ...
                  {currentPage > 3 && (
                    <>
                      <button
                        ...
                        className="min-w-[44px] h-11 bg-white border-2 border-teal-200 text-gray-600 font-semibold text-sm rounded-xl transition-all duration-300 hover:bg-teal-600 hover:text-white hover:-translate-y-1 hover:shadow-lg"
                      >
                        1
                      </button>
                      {currentPage > 4 && (
                  ...
                  {Array.from({ length: totalPages }, (_, index) => index + 1)
                    ...
                    .map((page) => (
                      <button
                        ...
                        className={`min-w-[44px] h-11 font-semibold text-sm rounded-xl transition-all duration-300 ${currentPage === page

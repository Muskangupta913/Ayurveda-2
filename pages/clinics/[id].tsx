...
const [showAuthModal, setShowAuthModal] = useState(false);
  ...
  useEffect(() => {
    ...
    const fetchClinic = async () => {
      try {
        setLoading(true);
        setError(null);
        const res = await axios.get<any>(`/api/clinics/${id}`); // 👈 `any` fixes TS error
        setClinic(res.data?.clinic || res.data?.data || res.data || null);
      } catch (err) {
  ...
  useEffect(() => {
    ...
    const fetchReviews = async () => {
      try {
        const res = await axios.get<any>(`/api/clinics/reviews/${clinic._id}`); // 👈 `any` here too
        if (res.data?.success) {
  ...
  const handleAuthSuccess = () => {
    setShowAuthModal(false);
    if (!clinic || !pendingAction) return;
                                ...
                                <div className="bg-gradient-to-br from-emerald-50 to-emerald-100/50 p-4 rounded-xl border border-emerald-200 hover:shadow-lg transition-all duration-300">
                                    <div className="flex items-center gap-3 mb-2">
                                        <div className="p-2 bg-emerald-200 rounded-lg">
                                            <span className="text-emerald-700 font-bold text-sm">AED</span>
                                        </div>
                                        <h3 className="font-bold text-gray-800">Consultation Fee</h3>
                                    </div>
                                    <p className="text-gray-700 font-medium text-lg">AED {clinic.pricing}</p>
                                </div>
                            )}
                        ...
                        {clinic.treatments && clinic.treatments.length > 0 && (
                            <div className="mb-8">
                                <div className="flex items-center gap-3 mb-6">
                                    ...
                                    <h3 className="text-xl font-bold text-gray-800">Specialized Treatments</h3>
                                </div>
                                        ...
                                        >
                                            ...
                                            {treatment.subTreatments && treatment.subTreatments.length > 0 && (
                                                <div className="ml-6 space-y-3">
                                                    <p className="text-sm font-medium text-gray-600 mb-3">Available Sub-treatments:</p>

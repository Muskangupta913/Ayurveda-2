...
const InvoiceManagementSystem = () => {
  ...
  useEffect(() => {
    ...
    if (!token) return;
    fetch("/api/clinic/patient-registration", {
  ...
  const fetchEMRData = useCallback(async () => {
    ...
    try {
      setFetching(true);
      const token = typeof window !== "undefined" ? localStorage.getItem("clinicToken") : null;
      const res = await fetch(`/api/clinic/patient-registration/${formData.emrNumber}`, {
        headers: { Authorization: `Bearer ${token}` }
      });
...
const handleSubmit = useCallback(async () => {
  ...
  setConfirmModal({
    ...
    action: async () => {
      try {
        const token = typeof window !== "undefined" ? localStorage.getItem("clinicToken") : null;
        const res = await fetch("/api/clinic/patient-registration", {
          method: "POST",
          headers: { 
        ...
        if (res.ok && data.success) {
          ...
          router.push("/clinic/staff/patient-information"); 
        } else {

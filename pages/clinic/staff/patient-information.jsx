...
function PatientFilterUI() {
  ...
  const fetchPatients = async () => {
    ...
    setLoading(true);
    try {
      const { data } = await axios.get("/api/clinic/get-patient-registrations", { params: filters, headers: { Authorization: `Bearer ${token}` } });
  ...
  const handleUpdate = (id) => router.push(`/clinic/staff/update-patient-info/${id}`);

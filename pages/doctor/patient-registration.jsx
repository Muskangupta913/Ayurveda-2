...
const InvoiceManagementSystem = () => {
  ...
  useEffect(() => {
    ...
    if (!token) return;
    fetch("/api/doctor/patient-registration", {
  ...
  const fetchEMRData = useCallback(async () => {
    ...
    try {
      setFetching(true);
      const token = typeof window !== "undefined" ? localStorage.getItem("doctorToken") : null;
      const res = await fetch(`/api/doctor/patient-registration/${formData.emrNumber}`, {
        headers: { Authorization: `Bearer ${token}` }
      });
...
const handleSubmit = useCallback(async () => {
  ...
  setConfirmModal({
    ...
    action: async () => {
      try {
        const token = typeof window !== "undefined" ? localStorage.getItem("doctorToken") : null;
        const res = await fetch("/api/doctor/patient-registration", {
          method: "POST",
          headers: { 
        ...
        if (res.ok && data.success) {
          ...
          router.push("/doctor/patient-information"); 
        } else {
...
const ProtectedDashboard = withDoctorAuth(InvoiceManagementSystem);

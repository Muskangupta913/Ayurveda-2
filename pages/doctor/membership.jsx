...
const MembershipModal = ({ isOpen, onClose }) => {
  ...
  const fetchByEmr = useCallback(async (overrideEmr) => {
    ...
    try {
      setFetching(true);
      const res = await fetch(`/api/doctor/patient-registration/${emrToFetch}`, {
  ...
  useEffect(() => {
    ...
    handle = setTimeout(async () => {
      try {
        const res = await fetch(`/api/doctor/emr-search?q=${encodeURIComponent(q)}`, {
          headers: { Authorization: `Bearer ${token}` }
        });
  ...
  const saveMembership = async () => {
    ...
    try {
      const res = await fetch('/api/doctor/members', {
        method: 'POST',
...
const Protected = withDoctorAuth(MembershipPage);

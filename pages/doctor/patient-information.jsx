...
function PatientFilterUI() {
  ...
  const fetchPatients = async () => {
    ...
    setLoading(true);
    try {
      const { data } = await axios.get("/api/doctor/get-patient-registrations", { params: filters, headers: { Authorization: `Bearer ${token}` } });
  ...
  const handleUpdate = (id) => router.push(`/doctor/update-patient-info/${id}`);
...
PatientFilterUI.getLayout = function PageLayout(page) { return <DoctorLayout>{page}</DoctorLayout>; };
const ProtectedDashboard = withDoctorAuth(PatientFilterUI);

...
const InvoiceUpdateSystem = () => {
  ...
  useEffect(() => {
    ...
    const fetchInvoice = async () => {
      ...
      try {
        const res = await fetch(`/api/doctor/get-patient-data/${id}`, {
    ...
    showConfirm(
      ...
      async () => {
        try {
          ...
          const res = await fetch(`/api/doctor/get-patient-data/${invoiceId}`, {
            method: "PUT",
            headers: { 
          ...
          if (res.ok) {
            ...
            if (formData.status === "Rejected" || formData.status === "Cancelled" || formData.status === "Released") {
              ...
              try {
                const res2 = await fetch(`/api/doctor/get-patient-data/${invoiceId}`, {
                  method: "PUT",
                  headers: {
                    "Content-Type": "application/json",
                    Authorization: `Bearer ${doctorToken}`
  ...
  }, [formData, invoiceInfo, doctorToken, currentUser.name]);
  ...
  }, [claimStatusModal, invoiceInfo, currentUser.name, formData.advanceClaimReleaseDate, formData.advanceClaimReleasedBy, doctorToken]);
  ...
  }, [formData.status, formData.rejectionNote, formData.advanceClaimReleaseDate, formData.advanceClaimReleasedBy, invoiceInfo, doctorToken, currentUser.name]);
...
const ProtectedDashboard = withDoctorAuth(InvoiceUpdateSystem);

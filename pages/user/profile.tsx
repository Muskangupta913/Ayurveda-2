...
// };
  ...
  interface Chat {
    _id: string;
    doctor: { _id: string; name: string; doctorProfileId?: string };
  ...
  const calculateStatistics = (
    ...
    comments: {
      ...
      withReplies: comments.filter((comment) => comment.replies.length > 0)
        .length,
    ...
    chats: {
      ...
      active: chats.filter(
        (chat) => chat.prescriptionRequest?.status === "active"
      ).length,
      completed: chats.filter(
        (chat) => chat.prescriptionRequest?.status === "completed"
      ).length,
    },
  });
  ...
  const JobCard = ({ application }: { application: AppliedJob }) => {
            ...
            <div className="space-y-3">
              {replies.map((reply) => {
                const isAuthorReply =
                  ...
                  reply.user &&
                  String(reply.user) ===
                  String((blogAuthor as { _id?: string })?._id || blogAuthor);
  ...
  const ChatCard = ({ chat, router }: { chat: Chat; router: { push: (url: string) => void } }) => (
    <div className="bg-[#2D9AA5]/5 border border-[#2D9AA5]/20 rounded-xl p-6 hover:bg-[#2D9AA5]/10 transition-colors">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                  ...
                  <p className="text-white">
                    {chat.prescriptionRequest.healthIssue}
                  </p>
                </div>
              ...
              {msg.messageType === "prescription" ? (
                <div>
                  <div className="flex items-center mb-2">
                    ...
                    <span className="font-medium text-sm text-[#2D9AA5]">
                      Prescription:
                    </span>
                  </div>
  ...
  const useUserProfile = () => {
    ...
    type MeResponse = { user?: ApiUser } | ApiUser;
    type ProfileResponse = { user?: ApiUser } | ApiUser;
    ...
    useEffect(() => {
      const fetchUser = async () => {
        ...
        try {
          ...
          const data = res.data;
          // Accept both { user: {...} } and {...} shapes
          const user = (data as { user?: ApiUser })?.user ?? (data as ApiUser);
          if (user && (user.name || user.email || user.phone)) {
            setUserProfile({ name: user.name ?? "", email: user.email ?? "", phone: user.phone });
            return;
          }
        } catch {
          // Silent fallthrough to optional fallback below
        }

        try {
          const apiBase = process.env.NEXT_PUBLIC_API_BASE_URL || "";
          // Fallback endpoint if the above is not available in your backend
          const res2 = await axios.get<ProfileResponse>(
            `${apiBase}/api/users/profile`,
            { headers: { Authorization: `Bearer ${token}` }, withCredentials: true }
          );
          const data2 = res2.data;
          const user2 = (data2 as { user?: ApiUser })?.user ?? (data2 as ApiUser);
          if (user2 && (user2.name || user2.email || user2.phone)) {
            setUserProfile({ name: user2.name ?? "", email: user2.email ?? "", phone: user2.phone });
            return;
          }
          setUserProfile(undefined);
        } catch (e) {
  ...
  const useNotifications = (router: { push: (url: string) => void }) => {
    ...
    const fetchNotifications = async () => {
      ...
      try {
        ...
        const response = await axios.get<{ notifications?: Notification[]; data?: Notification[] }>(
        ...
        if (error && typeof error === 'object' && 'response' in error) {
          ...
          if (axiosError.response?.status === 404) {
            setNotificationsError(
              "Notifications API not found. Feature may not be available."
            );
            setNotifications([]);
    ...
    const handleNotificationClick = async (notification: Notification) => {
      ...
      if (token) {
        try {
          ...
          await axios.post<{ success?: boolean }>(
            `${apiBase}/api/push-notification/mark-read`,
            {
      ...
      try {
        if (notification.type === "blog-reply" && notification.relatedBlog) {
          ...
          console.log(
            "No navigation configured for this notification type:",
            notification.type
          );
        }
  ...
  const useApiData = () => {
    ...
    const fetchAll = async () => {
      ...
      if (!token) {
        ...
        setChats([]);
        setCommentsError("You must be logged in to see your dashboard.");
    ...
    const renderChatsTab = () => (
      <div className="space-y-6">
        <div className="flex items-center space-x-3 mb-6">
          ...
          <h2 className="text-2xl font-bold text-white">
            Your Chat History with Doctors
          </h2>
        </div>
    ...
    return (
      <div className="min-h-screen bg-[#18232b]">
        ...
        <header className="bg-[#2D9AA5] shadow-lg sticky top-0 z-50">
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div className="flex justify-between items-center h-16">
              ...
              <div className="flex items-center space-x-4">
                {(userProfile && (userProfile.name || userProfile.email || userProfile.phone)) ||
                (applicantInfo && (applicantInfo.name || applicantInfo.email || applicantInfo.phone)) ? (
                  <div className="hidden md:flex flex-col items-end mr-2 pr-3 border-r border-white/20">
                    {(userProfile?.name || applicantInfo?.name) ? (
                      <span className="text-white text-sm font-medium">{userProfile?.name || applicantInfo?.name}</span>
                    ) : null}
                    <div className="flex items-center space-x-2 text-xs text-blue-100/90">
                      {(userProfile?.email || applicantInfo?.email) ? (
                        <span>{userProfile?.email || applicantInfo?.email}</span>
                      ) : null}
                      {(userProfile?.phone || applicantInfo?.phone) ? (
                        <span className="before:content-['•'] before:px-2">{userProfile?.phone || applicantInfo?.phone}</span>
                      ) : null}
                    </div>
                  </div>
                ) : null}
                ...
                >
                  ...
                  <span className="hidden sm:inline relative z-10 font-medium group-hover:text-blue-200 transition-colors">
                    Home
                  </span>
                </button>
                ...
                >
                  ...
                  <span className="hidden sm:inline relative z-10 font-medium group-hover:text-red-200 transition-colors">
                    Logout
                  </span>
                </button>
              </div>
        ...
        <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
          {/* Global Loader while any API is fetching */}
          {overallLoading ? (
            <div className="py-16">
              <LoadingSpinner text="Refreshing your dashboard..." />
            </div>
          ) : null}
          ...
          {!overallLoading && renderActiveTab()}
        </main>
      </div>
    );
  };

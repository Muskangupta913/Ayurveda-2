    ...
    switch (method) {
      case 'GET':
        try {
          ...
          if (id) {
            ...
            if (!blog) {
              return res.status(404).json({ success: false, message: 'Published blog not found or you lack permission' });
            }
        ...
        try {
          ...
          if (!title || !content) {
            return res.status(400).json({
              success: false,
              message: 'Title and content are required for published blogs'
            });
          }
          ...
          const populatedBlog = await Blog.findById(publishedBlog._id).populate('postedBy', 'name email');
        ...
        try {
          ...
          if (existingBlog.postedBy.toString() !== userId && role !== 'admin') {
            return res.status(403).json({
              success: false,
              message: 'You can only edit your own published blogs unless you are an admin'
            });
          }
      ...
      case 'DELETE':
        try {
          ...
          if (existingBlog.postedBy.toString() !== userId && !allowedRoles.delete.includes(role)) {
            return res.status(403).json({
              success: false,
          ...
          res.status(200).json({ success: true, message: 'Published blog deleted successfully' });
        } catch (error) {
          if (error.message.includes('No token') || error.message.includes('Authentication failed')) {
            return res.status(401).json({ success: false, message: error.message });
          }

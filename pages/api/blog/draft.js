    ...
    switch (method) {
      case 'GET':
        try {
          ...
          if (id) {
            ...
            if (!draft) {
              return res.status(404).json({ success: false, message: 'Draft not found or you lack permission' });
            }
      ...
      case 'POST':
        try {
          ...
          const populatedDraft = await Blog.findById(draft._id).populate('postedBy', 'name email');
      ...
      case 'PUT':
        try {
          ...
          if (existingDraft.postedBy.toString() !== userId && role !== 'admin') {
      ...
      case 'DELETE':
        try {
          ...
          if (existingDraft.postedBy.toString() !== userId && !allowedRoles.delete.includes(role)) {
            return res.status(403).json({
              success: false,

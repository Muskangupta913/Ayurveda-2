...
export default async function handler(req, res) {
  ...
  if (req.method === "POST") {
    try {
      // Check agent permissions for creating membership
      // Membership is a submodule under staff_management
      if (user.role === 'agent' || user.role === 'doctorStaff') {
        const hasPermission = await checkAgentPermission(
          ...
          'create',
          'Membership'
        ) || await checkAgentPermission(user._id, 'staff_management', 'all', 'Membership')
        || await checkAgentPermission(user._id, 'clinic_staff_management', 'create', 'Membership')
        || await checkAgentPermission(user._id, 'clinic_staff_management', 'all', 'Membership');
        
        if (!hasPermission) {
          return res.status(403).json({ 
            success: false, 
            message: "You don't have permission to create memberships" 
          });
        }
      }
  ...
  if (req.method === "GET") {
    try {
      // Check agent permissions for reading memberships
      // Membership is a submodule under staff_management
      if (user.role === 'agent' || user.role === 'doctorStaff') {
        console.log(`Checking read permission for user ${user._id} on membership submodule`);
        const hasPermission = await checkAgentPermission(
          user._id, 
          'staff_management', 
          'read',
        ...
        || await checkAgentPermission(user._id, 'clinic_staff_management', 'read', 'Membership')
        || await checkAgentPermission(user._id, 'clinic_staff_management', 'all', 'Membership');
        
        console.log(`Read permission result for membership:`, hasPermission);
        
        if (!hasPermission) {
          return res.status(403).json({ 
            success: false, 
            message: "You don't have permission to view memberships" 
          });
        }
      }
  ...
  if (req.method === "PUT") {
    try {
      // Check agent permissions for updating membership
      // Membership is a submodule under staff_management
      if (user.role === 'agent' || user.role === 'doctorStaff') {
        const hasPermission = await checkAgentPermission(
          user._id, 
        ...
        || await checkAgentPermission(user._id, 'clinic_staff_management', 'update', 'Membership')
        || await checkAgentPermission(user._id, 'clinic_staff_management', 'all', 'Membership');
        
        if (!hasPermission) {
          return res.status(403).json({ 
            success: false, 
            message: "You don't have permission to update memberships" 
          });
        }
      }
  ...
  if (req.method === "PATCH") {
    ...
    try {
      // Check agent permissions for updating/transferring membership
      // Membership is a submodule under staff_management
      if (user.role === 'agent' || user.role === 'doctorStaff') {
        const hasPermission = await checkAgentPermission(
          user._id, 
        ...
        if (!hasPermission) {
          return res.status(403).json({ 
            success: false, 
            message: "You don't have permission to transfer memberships" 
          });
        }
      }
  ...
  if (req.method === "DELETE") {
    try {
      // Check agent permissions for deleting membership
      // Membership is a submodule under staff_management
      if (user.role === 'agent' || user.role === 'doctorStaff') {
        const hasPermission = await checkAgentPermission(
          user._id, 
        ...
        || await checkAgentPermission(user._id, 'clinic_staff_management', 'delete', 'Membership')
        || await checkAgentPermission(user._id, 'clinic_staff_management', 'all', 'Membership');
        
        if (!hasPermission) {
          return res.status(403).json({ 
            success: false, 
            message: "You don't have permission to delete memberships" 
          });
        }
      }
